...
def process_lines(lines: list[str]):
    ...
    for u, cu in enumerate(tqdm(cuboids)):
        ...
        originalG.add_node(u, h=uh)
        for v, cv in enumerate(cuboids):
            # Is v original u or at the same level?
            ...
            originalG.add_edge(u, v)

    # Which bricks are on the floor?
    underG = originalG.copy()
    ...
    nodes_and_zs = set((n, d["z"]) for n, d in underG.nodes(data=True))
    n_total = 0
    for remove_n in tqdm(underG.nodes):
        G = originalG.copy()
        G.remove_node(remove_n)

        to_process = set(n for n in G.nodes if len(list(G.predecessors(n))) == 0)
        for n in to_process:
            G.nodes[n]["z"] = 1
        ...
        remove_nodes_and_zs = set((n, d["z"]) for n, d in G.nodes(data=True))
        n_total += len(remove_nodes_and_zs - nodes_and_zs)
    print(n_total)
